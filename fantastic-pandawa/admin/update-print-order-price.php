<?php
// File: update-print-order-price.php
session_start();
...
function logDebug($message, $data = null) {
    $debug_file = '../logs/print-price-update-' . date('Y-m-d') . '.log';
...
// Log initial request
logDebug("Print price update request started", [
    'POST' => $_POST,
    'SESSION' => $_SESSION
]);
...
try {
    // Update price
    $stmt = $conn->prepare("UPDATE print_orders SET price = ?, updated_at = NOW() WHERE id = ?");
